...
lemma support_bind_prod_mk_snd : (oa >>= λ a, return (b, g a)).support =
  (λ x, (b, g x)) '' oa.support := support_bind_return oa _
...
lemma mem_support_bind_prod_mk_fst_of_subsingleton [subsingleton γ]
  (x : β × γ) : x ∈ (oa >>= λ a, return (f a, c)).support ↔ ∃ a ∈ oa.support, f a = x.1 :=
...
lemma mem_support_bind_prod_mk_snd_of_subsingleton [subsingleton β]
  (x : β × γ) : x ∈ (oa >>= λ a, return (b, g a)).support ↔ ∃ a ∈ oa.support, g a = x.2 :=
by simp_rw [support_bind_prod_mk_of_fst_subsingleton, set.mem_preimage, set.mem_image, exists_prop]

lemma support_bind_prod_mk_fst_id_of_subsingleton [subsingleton γ] :
  (oa >>= λ a, return (a, c)).support = prod.fst ⁻¹' oa.support :=
by rw [support_bind_prod_mk_of_snd_subsingleton, set.image_id']

lemma support_bind_prod_mk_snd_id_of_subsingleton [subsingleton β] :
  (oa >>= λ a, return (b, a)).support = prod.snd ⁻¹' oa.support :=
by simp only [support_bind_prod_mk_of_fst_subsingleton, set.image_id']

lemma mem_support_bind_prod_mk_fst_id_of_subsingleton [subsingleton γ] (x : α × γ) :
  x ∈ (oa >>= λ a, return (a, c)).support ↔ x.1 ∈ oa.support :=
by rw [support_bind_prod_mk_fst_id_of_subsingleton, set.mem_preimage]

lemma mem_support_bind_prod_mk_snd_id_of_subsingleton [subsingleton β] (x : β × α) :
  x ∈ (oa >>= λ a, return (b, a)).support ↔ x.2 ∈ oa.support :=
by rw [support_bind_prod_mk_snd_id_of_subsingleton, set.mem_preimage]
...
lemma fin_support_bind_prod_mk_snd [decidable_eq γ] :
  (oa >>= λ a, return (b, g a)).fin_support = oa.fin_support.image (λ x, (b, g x)) :=
