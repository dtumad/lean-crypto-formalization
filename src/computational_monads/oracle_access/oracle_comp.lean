...
lemma simulate_oc_bind (so : simulation_oracle spec)
  ...
  simulate so (oc_bind oc od) s = 
    do {cs ← simulate so oc s, simulate so (od cs.1) cs.2} :=
congr_arg (λ x, simulate so oc s >>= x) (funext $ prod.rec $ by simp)
...
def logging_simulation_oracle {C : Type} (oc : oracle_comp spec C)
