...
if (il i).empty then (λ t, (t, il)) <$> oa else return ((il i).head, il.drop_at_index i 1)
...
lemma get_or_else_of_eq_cons (t ts) (h : il i = t :: ts) :
  il.get_or_else i oa = return (t, il.drop_at_index i 1) := by simp [get_or_else, h]
