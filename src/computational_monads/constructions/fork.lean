...
-- lemma prob_event_value_differs (adv : fork_adversary spec α β i q) :
--   ⁅λ fr, indexed_list.value_differs fr.seed₁ fr.seed₂ i fr.get_cf | fork adv y⁆ =
--     1 - (fintype.card (spec.range i))⁻¹ :=
-- let i' : spec.ι := sum.inr i in
-- calc ⁅λ fr, indexed_list.value_differs fr.seed₁ fr.seed₂ i fr.get_cf | fork adv y⁆ =
...
-- lemma prob_event_chosen_fork_ne_none_and_eq (adv : fork_adversary spec α β i q) :
...
--         * (possible_outcomes (adv.qb.decrement i fp))⁻¹) ^ 2 :
--     begin
...
--         * (possible_outcomes (adv.qb.decrement i fp))⁻¹) :
--     begin
...
-- theorem prob_event_ne_none_le_prob_event_fork_success (adv : fork_adversary spec α β i q) :
...
{ run := λ y, simulate loggingₛₒ (adv.run y) ∅,
  choose_fork := λ y z, let inp := choose_input y z.1 in
    let ts : list (spec.domain i × spec.range i) := (z.2 i) in
