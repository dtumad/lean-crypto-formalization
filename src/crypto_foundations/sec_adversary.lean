...
(run : α → prob_comp adv_spec β)
(qb : query_count (uniform_selecting ++ adv_spec))
...
(inp_gen : prob_comp exp_spec (α × γ))
(is_valid : α × γ → β × S → prob_comp exp_spec bool)
...
def prob_sim_oracle {spec spec' : oracle_spec} {S : Type}
  (so : sim_oracle spec spec' S) :
  sim_oracle (uniform_selecting ++ spec) (uniform_selecting ++ spec') S :=
sorry

noncomputable def execute (adv : sec_adversary adv_spec α β)
  (exp : sec_experiment adv_spec exp_spec α β γ S S')
  (x : α × γ) :
  prob_comp exp_spec (β × S) :=
dsimulate (prob_sim_oracle $ exp.adv_so x) (adv.run x.1)
...
⁅= tt | dsimulate' (idₛₒ ++ₛ exp.exp_so)
  (do {x ← exp.inp_gen, z ← execute adv exp x, exp.is_valid x z})⁆
