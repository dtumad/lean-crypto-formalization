...
def vectorization [homogenous_space G X] (x y : X) : G :=
fintype.choose _ (transitive_action_unique x y)

local notation `δ` := vectorization

lemma vectorization_smul_eq [homogenous_space G X] (x y : X) :
  (δ x y : G) • x = y :=
...
variables {G X : Type} [fintype G] [fintype X] [inhabited X] 
  [decidable_eq X] [decidable_eq G] [homogenous_space G X]
...
noncomputable def parallelization_advantage 
  ...
  [homogenous_space G X](f : X × X × X → comp X) 
